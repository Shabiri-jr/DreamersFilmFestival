...
Six organiser-supplied landmarks appear as numbered pins. Select a pin or its name to view details and its original Google Maps link; **Landmarks** restores the overview. Super Admins can rename every pin under **Admin → Map landmarks** (`/admin/landmarks`). Names save to `venue_landmark_labels` and update both public maps. Apply migration `202609230014_venue_landmark_labels.sql` before using the editor. Coordinates and source links remain in `src/lib/venue/landmarks.ts`; coordinate-only links start with numbered labels until the organiser names them.
